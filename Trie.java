...
public class Trie{
    ...
    private boolean estaVacio(TrieNode root){          // O(1)
        for (int i = 0; i < 256; i++){                 // O(1)
            if (root.hijos[i] != null){return false;}  // O(1)
        }
        return true;                                   // O(1)
    }
    ...
    /* Lo que hace este metodo es basicamente ir recorriendo el trie de manera lexicografica [0,1,...,255,256]
     * ingresa con un prefijo "" que es un String vacio, y lo va a ir llenando con el caracter correspondiente
     * a su posicion en el Trie. Una vez terminado, vuelve al primer trie que tuviese mas de un hijo y continua
     * con ese, hasta terminar con todo el Trie, para luego volver al anterior y asi sucesivamente hasta que  
     * llegue al ultimo elemento del Trie root. Es recursivo. 
     * Para terminar, su complejidad va a depender de que estemos buscando, si en el Trie de Carreras o en el
     * Trie de materias, es por eso que esta anotado: O(c∈C Σ|c|) || O(mc∈Mc Σ|mc|)  */
    private void obtenerClavesRec(TrieNode nodo, String prefijo, ArrayList<String> claves) { // O(c∈C Σ|c|) || O(mc∈Mc Σ|mc|) 
        if (nodo == null) {return;}                                                          // O(1)
        if (nodo.valor != null) {claves.add(prefijo);}                                       // O(1)   
    ...
    /*Casi lo mismo que obtenerClavesRec pero solo vamos a ir a una palabra y a la vuelta le vamos a ir sacando 
    ...
    *   / 
    *  a
    * eliminar "pata" iria hasta la ultima letra y al volver recursivamente eliminaria el nodo a, t y se pararia
    * en el nodo de la letra a que indica que hay otra palabra, en este caso "par".
    * Como esta funcion va a ser unicamente utilizada para cerrarMaterias, su complejidad toma unicamente
    * |m| = nombre de la materia. */
    private void eliminarRec(TrieNode nodo, String palabra, int index) { // O(|m|)
        if (nodo == null) {return;}                                      // O(1)

        if (index == palabra.length()) {                                 // O(1)
            if (nodo.valor != null) {nodo.valor = null;}                 // O(1)
            return;                                                      // O(1)
        }
